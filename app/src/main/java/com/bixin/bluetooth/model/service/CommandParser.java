...
import com.bixin.bluetooth.model.event.BackgroundCurrentNumberEvent;
import com.bixin.bluetooth.model.event.BackgroundHfpStatusEvent;
import com.bixin.bluetooth.view.activity.TransparentActivity;
...
public class CommandParser {
	...
	private void handleWithoutCallback(String cmd){
        Log.d(TAG, "handleWithoutCallback:cmd "+cmd);
			...
			try {
				if (cmd.startsWith(Commands.IND_HFP_CONNECTED)) {
					...
					if (cmd.length() < 3) {
                        ...
                        Log.d(TAG, "onSerialCommand: cmd " + cmd);
					...
					if (cmd.length() < 6) {
						...
						if (cmd.contains("[FF]")) {
                            ...
                            Log.d(TAG, "onSerialCommand:nameLen "+nameLen+" numLen "+numLen+" bytes "+bytes.length+"\n"+cmd );
                            if (nameLen > 0) {
